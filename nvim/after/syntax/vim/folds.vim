...
" Vim's command window and options window also have ft=vim, but we don't want those
" Regex match, match case
if bufname('') =~# '^\%(' . (v:version < 702 ? 'command-line' : '\[Command Line\]') . '\|option-window\)$'
	setlocal nofoldenable
endif

function! VimLFoldText()
	let l = '  ' . v:folddashes . ' ' . getline(v:foldstart) . ': ' . (v:foldend - v:foldstart) . ' lines'
	return l
endfunction

setlocal foldtext=VimLFoldText()
